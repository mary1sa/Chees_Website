...
const Home = () => {
  ...
  useEffect(() => {
    ...
    // Logo animation properties - MODIFIED to keep logo upright
    // Modify the logoPositions array to include top percentage
const logoPositions = [
  ...
  { left: 50, top: 90, size: 200, scale: 0.5 }   // Section 4: Center again, but smaller 
];
    
    // Create scroll snap points for each section
    ...
    sections.forEach((section, index) => {
      ...
      // Create a transition for this section - MODIFIED to use scale and avoid rotation
      // In the mainTimeline.to() function
      mainTimeline.to(logoRef.current, {
        left: `${nextPos.left}%`,
        top: `${nextPos.top}%`,  // Add this line to animate top position
        width: nextPos.size,
        height: nextPos.size,
        scale: nextPos.scale,
        ease: "power1.inOut",
        duration: scrollDuration,
        // Remove or modify the onUpdate function since we're now animating the top position
      }, `section${index}`);
    ...
    // Add subtle logo effects for each section transition
    sections.forEach((section, index) => {
      if (!section) return;
      
      // Create a subtle pulsing effect when entering each section
      gsap.timeline({
        scrollTrigger: {
          trigger: section,
          start: 'top center',
          ...
          onEnter: () => {
            gsap.to(logoRef.current, {
              scale: logoPositions[index].scale * 1.05, // Slight increase in scale
              duration: 0.5,
              ease: "elastic.out(1, 0.3)",
              onComplete: () => {
                gsap.to(logoRef.current, {
                  scale: logoPositions[index].scale,
                  duration: 0.3,
                  ease: "power2.out"
                });
              }
    ...
    // Additional section-specific content animations
    // Section 1
    gsap.timeline({
      scrollTrigger: {
    ...
    // Smoother scrolling with ScrollToPlugin for better experience
    ...
    // Optional: Add a reactive shadow effect to the logo based on scroll position
    const updateLogoEffect = () => {
      ...
      // Apply a subtle shadow or glow effect based on current section
      ...
      // Apply different shadow effects for each section
      const shadowColors = [
        'rgba(26, 60, 94, 0.3)',     // Section 1: Blue shade
        'rgba(233, 224, 209, 0.3)',   // Section 2: Beige shade
        'rgba(141, 169, 196, 0.3)',   // Section 3: Light blue shade
        'rgba(255, 255, 255, 0.3)'      // Section 4: Red shade
      ];
      
      gsap.to(logoRef.current, {
        boxShadow: `0 0 30px 5px ${shadowColors[currentSection]}`,
        duration: 0.5,
        ease: "power1.out"
      });
    };
    ...
    <div className="home-container" ref={homeContainerRef}>
      ...
      <section className="section section-3" ref={section3Ref}>
        <div className="left-aligned-content">
          <div className="content-wrapper">
            <div className="image-wrapper content-item">
      ...
      <section className="section section-4" ref={section4Ref}>
        <h2 className="player-header">Our Distinguished Players</h2>
        <div className="content-wrapper">
          ...
          <div className="player-showcase">
            ...
            <p>1st place in the national championship 2022 in classical chess for under 8 years old</p>
